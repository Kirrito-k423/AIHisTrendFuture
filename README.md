...
- **114 个时间节点**，其中包含 57 个可横向比较的训练技术节点；新增 5 条 DeepSpeed 专栏泳道，按官方教程 Git 首次提交时间纳入 21 个方法。从 T2I/T2V/Omni 综述逐条核验的 32 个候选条目（29 个模型、3 个方法）仍保留原证据状态，任何未查到的项目都显式标为“未知”。
- **当前快照**：SenseNova-U1.5-8B-MoT Preview（开放权重 / native 4K 图像生成编辑）、OpenAI Astra（内部模型 / 数学形式化证书）、DeepSeek-V4-Flash-0731（开放权重 / 13B active）、MiniMax H3（2K 原生立体声音视频 / 待开放权重）、Seedance 2.5（30 秒长叙事 / 多参考编辑）、Inkling-Small（开放权重 / 276B MoE）、MAI-Cyber-1-Flash、Claude Opus 5、FLUX 3（Early Access / 待发布）、Kimi K3（开放权重 / 2.8T MoE）、Qwen-Audio-3.0-TTS、Gemini 3.6 Flash、GLM‑5.2、MiniMax‑M3、DeepSeek‑V4‑Pro、Kimi K2.6、Qwen3.6、Cosmos3‑Super、Cosmos3‑Edge、ABot‑World‑0 与 LTX‑2.3；旧模型作为架构和性能基线。
- **模型对比**：覆盖从 2022‑11 ChatGPT 到 2026‑08 的 99 个代表模型，并可按 T2I/T2V/Omni 筛选。除 LLM 指标外，还提供生成 Elo、单次生成时间、输出规格及逐项来源。
