...
import type { StarNarrative, TechnologyCategory, TechnologyComparison, TimelineEvent } from "../types";
...
function ComparisonMatrix({ models }: { models: TechnologyEvent[] }) {
  const rows: Array<[string, (event: TechnologyEvent) => ReactNode]> = [
    ["分类 / 发布", (event) => <><strong>{event.comparison.category}</strong><small>{event.date} · {event.organization}</small></>],
...
export function TechnologyComparison({ hubNavigation }: { hubNavigation?: ReactNode }) {
  ...
  const visible = useMemo(() => {
    ...
    return technologies.filter((event) => {
      ...
      const haystack = [event.title, event.organization, event.summary, event.comparison.mechanism, event.comparison.bestFor, ...event.tags].join(" ").toLowerCase();
    ...
    <main className="atlas technology-comparison-page">
      ...
      <section className="comparison-summary technology-summary">
        <div>
          ...
          <h1>训练技术横向比较</h1>
          <p>把结构、后训练算法、kernel 与分布式系统放在同一张矩阵中，但不混淆指标口径。每项都能回到论文、官方博客或代码仓，并按 STAR 说明为什么做、如何验证、得到什么结果。</p>
        </div>
        <dl>
          <div><dt>技术</dt><dd>{technologies.length}</dd></div>
          <div><dt>类别</dt><dd>4</dd></div>
      ...
      <section className="technology-controls" aria-label="训练技术筛选">
        ...
        <div className="technology-category-filter">{categories.map((item) => <button type="button" key={item} className={category === item ? "active" : ""} onClick={() => setCategory(item)}>{item}</button>)}</div>
      ...
      <section className="technology-catalog" aria-label="训练技术目录">
        {visible.map((event) => (
          <article key={event.id} className={selectedIds.includes(event.id) ? "selected" : ""}>
            <header><span>{event.comparison.category}</span><time>{event.date}</time></header>
            <h2>{event.title}</h2>
            <p>{event.summary}</p>
