...
test("keeps the fixed research schema and explicit unknown values", async () => {
  ...
  assert.equal((source.match(/modelEvent\(\{/g) ?? []).length, 57);
  ...
  assert.match(source, /const claudeFable51 = modelEvent/);
  ...
  assert.match(source, /Gemini 3\.8 Flash Cyber/);
  assert.match(source, /Muse Spark 1\.3/);
...
test("comparison page renders source-backed structured text fields side by side", async () => {
  ...
  assert.match(html, /Gemini 3\.8 Flash Cyber/);
