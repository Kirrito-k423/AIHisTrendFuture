...
- **当前快照**：HPC-Ops × SGLang（H20/H200 MoE 推理算子 / Hy3 TPOT 15.1–48.8% 下降）、Categorical Flow Maps（1.7B / 2.1T tokens / 4-step language generation）、Ling-3.0-flash（开放权重 / 124B MoE / 35 KDA + 7 Gated MLA）、Qwen3.8-Max（2.4T MoE API / 指标未披露）、NVIDIA Alpamayo 2 Super（开放权重 / AV VLA）、SpecForge v0.3（分离式 speculative decoding 训练/服务栈）、SenseNova-U1.5-8B-MoT Preview（开放权重 / native 4K 图像生成编辑）、OpenAI Astra（内部模型 / 数学形式化证书）、DeepSeek-V4-Flash-0731（开放权重 / 13B active）、MiniMax H3（2K 原生立体声音视频 / 待开放权重）、Seedance 2.5（30 秒长叙事 / 多参考编辑）、Inkling-Small（开放权重 / 276B MoE）、MAI-Cyber-1-Flash、Claude Opus 5、FLUX 3（Early Access / 待发布）、Kimi K3（开放权重 / 2.8T MoE）、Qwen-Audio-3.0-TTS、Gemini 3.6 Flash、GLM‑5.2、MiniMax‑M3、DeepSeek‑V4‑Pro、Kimi K2.6、Qwen3.6、Cosmos3‑Super、Cosmos3‑Edge、ABot‑World‑0 与 LTX‑2.3；旧模型作为架构和性能基线。
