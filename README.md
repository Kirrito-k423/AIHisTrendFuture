...
- **124 个时间节点**，其中包含 60 个可横向比较的训练技术节点；新增 5 条 DeepSpeed 专栏泳道，按官方教程 Git 首次提交时间纳入 21 个方法。从 T2I/T2V/Omni 综述逐条核验的 32 个候选条目（29 个模型、3 个方法）仍保留原证据状态，任何未查到的项目都显式标为“未知”。
- **当前快照**：Muse Glimmer 30B（Apache-2.0 / 29.6B / 131K / SGLang day-0）、NemotronLabs VoiceChat 11B（开放权重 / 全双工语音 / tool calling / 448 ms）、Claude zeta research model（内部模型 / zeta zeros ≥67.2% / Lean）、GPT-5.6-Cyber（Daybreak Red / cyber 专用模型 / 指标未披露）、DeepSpeed v0.19.5 `pin_memory` 修订、HPC-Ops × SGLang（H20/H200 MoE 推理算子 / Hy3 TPOT 15.1–48.8% 下降）、Categorical Flow Maps（1.7B / 2.1T tokens / 4-step language generation）、Ling-3.0-flash（开放权重 / 124B MoE / 35 KDA + 7 Gated MLA）、Qwen3.8-Max（2.4T MoE API / 指标未披露）、NVIDIA Alpamayo 2 Super（开放权重 / AV VLA）、SpecForge v0.3（分离式 speculative decoding 训练/服务栈）、SenseNova-U1.5-8B-MoT Preview、OpenAI Astra、DeepSeek-V4-Flash-0731、Inkling-Small、MAI-Cyber-1-Flash、Claude Opus 5、Kimi K3、Qwen-Audio-3.0-TTS、Gemini 3.6 Flash、GLM‑5.2、MiniMax‑M3、DeepSeek‑V4‑Pro、Qwen3.6 与 Cosmos3‑Edge；旧模型作为架构和性能基线。
- **模型对比**：覆盖从 2022‑11 ChatGPT 到 2026‑08 的 106 个代表模型，并可按 T2I/T2V/Omni 筛选。除 LLM 指标外，还提供生成 Elo、单次生成时间、输出规格及逐项来源。
